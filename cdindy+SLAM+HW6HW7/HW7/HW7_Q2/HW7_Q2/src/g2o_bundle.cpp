...
void BuildProblem(const BALProblem* bal_problem, g2o::SparseOptimizer* optimizer, const BundleParams& params)
{
    ...
    for(int i = 0; i < num_cameras; ++i)
    {
        ...
        CameraBAL cameraBAL(temVecCamera);
        pCamera->setEstimate(cameraBAL);   // initial value for the camera i..
...
void WriteToBALProblem(BALProblem* bal_problem, g2o::SparseOptimizer* optimizer)
{
    ...
    for(int i = 0; i < num_cameras; ++i)
    {
        ...
        Eigen::Matrix<double,6,1> se3 = pCamera->estimate().mSE3.log(); //estimate -> class CameraBAL
        Eigen::Matrix<double,9,1> NewCameraVec ;
        NewCameraVec << se3[3], se3[4], se3[5], se3[0], se3[1], se3[2],
            pCamera->estimate().mf, pCamera->estimate().mk1, pCamera->estimate().mk2;
